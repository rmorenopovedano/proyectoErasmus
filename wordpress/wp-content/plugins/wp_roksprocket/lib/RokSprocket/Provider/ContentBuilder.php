...
class RokSprocket_Provider_ContentBuilder extends RokSprocket_Provider_AbstarctJoomlaBasedProvider
{
    ...
    protected function convertRawToItem($raw_item, $dborder = 0)
    {
        ...
        $primary_link->setUrl(JRoute::_('login.php?option=com_contentbuilder&title=list-view-easyblog&controller=details&id=' . $raw_item->form_id . '&record_id=' . $raw_item->record_id));
    ...
    protected function getArticleEditUrl($id)
    {
        ...
        return JURI::root(true) . '/administrator/login.php?option=com_contentbuilder&controller=details&id=' . $form_id . '&record_id=' . $id;
    }
