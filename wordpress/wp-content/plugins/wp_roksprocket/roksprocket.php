...
if (!defined('ROKSPROCKET_VERSION')) define('ROKSPROCKET_VERSION', '2.1.14');
...
if (!preg_match('/project.version/', ROKCOMMON) && version_compare(preg_replace('/-SNAPSHOT/', '', ROKCOMMON), ROKSPROCKET_NEEDED_ROKCOMMON_VERSION, '<')) {
	rs_force_deactivate();
	roksprocket_set_admin_message('error', __('RokSprocket v2.1.14 needs at least RokCommon Version ' . ROKSPROCKET_NEEDED_ROKCOMMON_VERSION . '.  You currently have RokCommon Version ' . ROKCOMMON));
	return;
}
...
if (!function_exists('roksprocket_activate')) {
	...
	function roksprocket_activate()
	{
		...
		$file = ROKSPROCKET_PLUGIN_PATH . '/install/install.mysql.utf8.sql.sql';
...
if (!function_exists('roksprocket_uninstall')) {
	...
	function roksprocket_uninstall()
	{
		...
		$file = ROKSPROCKET_PLUGIN_URL . '/install/uninstall.mysql.utf8.sql.sql';
