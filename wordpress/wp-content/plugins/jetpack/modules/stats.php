...
function stats_merged_widget_admin_init() {
	if ( current_user_can( 'view_stats' ) ) {
		add_action( 'load-login.php', 'stats_enqueue_dashboard_head' );
...
function stats_admin_menu() {
	...
	if ( 'login.php' == $pagenow && isset( $_GET['page'] ) && 'stats' == $_GET['page'] ) {
		$redirect_url =	str_replace( array( '/wp-admin/login.php?', '/wp-admin/?' ), '/wp-admin/admin.php?', $_SERVER['REQUEST_URI'] );
...
<?php
	...
	foreach ( $args as $var => $vals ) {
		...
		if ( is_array( $vals ) ) {
			...
			if ( substr( $_REQUEST[$var], 0, 9 ) == 'login.php' )
				$q[$var] = $_REQUEST[$var];
		}
	}
	...
	if ( isset( $_GET['chart'] ) ) {
		...
		$url = 'https://' . STATS_DASHBOARD_SERVER . "/wp-admin/login.php";
	}
...
function stats_convert_admin_urls( $html ) {
	return str_replace( 'login.php?page=stats', 'admin.php?page=stats', $html );
}
