...
$consulta="select * from participantes where idConvocatoria=:idConvocatoria and validado=0 and tipo='profesor'";
$parametros=array("idConvocatoria"=>$idConvocatoria);
$result=$db->consulta($consulta,$parametros);
if($result){
    echo '<div class="table-scroll">';
    echo '<form action="gestionarValidacionesProfesor.php" method="post">';
    echo '<table class="hover centrar">';
    echo '<tr><th>Profesor</th><th>Ciclo</th><th>Validar</th></tr>';
    ...
    echo '<div class="alert callout centrar">
              <p><i class="fi-alert"></i>No hay profesores que validar esta convocatoria.</p>
          </div>';
}